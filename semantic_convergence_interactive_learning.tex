...
\begin{abstract}
We seek sufficient conditions under which an interactive learner's posterior concentrates on the \emph{interventional semantic class} $[p^\star]$ of an unknown computable environment --- the programs that induce the same action-conditioned law as $p^\star$ under every admissible intervention, and the strongest target any history-based learner can reach. The nested chain $\{p^\star\}\subseteq[p^\star]\subseteq R_\pi(p^\star)\subseteq R_{h_t}(p^\star)$ collects the indistinguishability classes of four \emph{observers} at increasing observational power, and each learning rule in the paper is an observer-promotion attempt. A natural means is the \emph{algorithmic free energy}: the Gibbs-form variational functional whose reference measure is the Kolmogorov-complexity-weighted universal interactive prior of \citet{Solomonoff1964a,Solomonoff1964b,Kolmogorov1965,Hutter2005}, rather than the stipulated parametric prior of active-inference free energy. On this functional, two unifications are exact: its minimization is Bayesian updating with the KL-based divergence forced within the natural convex class; and at zero action cost with uniform preferences, the resulting expected free energy is the information-gain criterion shared by Bayesian experimental design, compression-based curiosity, and active-inference curiosity. Universal prior, Bayes/Gibbs belief, and expected-free-energy-minimizing action together form the \emph{algorithmic free energy principle}. Even so, a near-miss theorem for a substituted finite-support prior freezes the posterior on $[p^\star]$ at prior mass for every horizon; because its action rule is exactly the unifying information-gain criterion, the failure is of generic information-seeking itself. A single sufficiency theorem answers the question: under the universal prior, Bayes/Gibbs belief, semantic separation, and divergent cumulative policy mass on separating actions, the posterior on $[p^\star]$ converges almost surely to one, with explicit finite-time high-probability lower bounds under bounded log-likelihood ratios. The condition is sharp --- zero separating support makes uniform recovery impossible and summable separating support is insufficient --- and is attained by three canonical realizations under the standing finite-action setup: the class-against-complement selector, the exact Gibbs kernel lift, and a differentiable finite-latent-class surrogate; a divergent exploration budget alone also suffices. Corollary~\ref{cor:compact-action-kernel} extends the exact kernel route to compact metric action spaces under action-continuity on the Bhattacharyya score and a full-support Borel reference measure. A public Lean 4 repository at \url{https://github.com/Populustremuloides/semantic_convergence} contains Lean counterparts for all 106 manuscript items over the explicit concrete foundation used in the repo, together with generated coverage, proof-shape, and per-declaration axiom audits; the generated manifest and audit now certify semantic-audit closure and first-principles closure. On the belief and variational side, the Lean declarations \path{SemanticConvergence.def_afe}, \path{SemanticConvergence.def_two_observer_functional}, and \path{SemanticConvergence.def_kernel_functional} now implement the repaired generalized-KL / Gibbs objectives rather than reduced proxy products, while \path{SemanticConvergence.lem_variational}, \path{SemanticConvergence.prop_two_observer_minimizer}, and \path{SemanticConvergence.prop_kernel_functional_minimizer} certify the corresponding exact Bayes/Gibbs minimizers on the paper-facing countable surface. Around Section~\ref{sec:main}, the Lean declarations \path{SemanticConvergence.thm_separating_support_convergence}, \path{SemanticConvergence.thm_separating_support_rate}, and \path{SemanticConvergence.cor_separating_support_finite_time} now realize the probabilistic support/rate stack on the realized trajectory law directly and derive it from the deterministic concrete substrate through an explicit Lean bridge, with no external bridge-equation hypothesis left on the public theorem surface: the first proves an almost-sure one-step residual-odds contraction on the realized process, the second upgrades it to an almost-sure positive-floor $N$-step residual-odds rate bound, and the third converts that bound into an almost-sure lower bound on the realized observer-fiber posterior share. At the boundary and surrogate end, the Lean declarations \path{SemanticConvergence.thm_afe_near_miss}, \path{SemanticConvergence.thm_amortized_surrogate_selector_existence}, and \path{SemanticConvergence.thm_amortized_surrogate} now match the repaired paper-facing theorem surfaces: the first packages the action-level near-miss witness together with the finite-horizon frozen-posterior failure shape, the second gives the countable finite-list wrapper carrying counterparts of assumptions \textup{(A1)--(A3)} to an implemented-law support conclusion, and the third derives the deployment-side separating-support floor used by the recovery stack.
\end{abstract}
...
\emph{Observer promotion} is the organizing informal concept: each learning rule attempts to push the realized observer toward the behavioral one, and the four-observer hierarchy grades that attempt. The self-referential reduction (Section~\ref{sec:self-ref}) replaces the uncomputable behavioral observer by a finite-time proxy and recovers the main guarantee under eventual isolation (deterministic splitting or divergent exploration). The public Lean 4 repository at \url{https://github.com/Populustremuloides/semantic_convergence} contains counterparts for all 106 manuscript items over the explicit concrete foundation used in the repo, and publishes generated coverage, proof-shape, and per-declaration axiom audits documenting the verification state; the generated manifest and audit now certify semantic-audit closure and first-principles closure. On the variational side, the Lean declarations \path{SemanticConvergence.def_afe}, \path{SemanticConvergence.def_two_observer_functional}, and \path{SemanticConvergence.def_kernel_functional} now realize the repaired generalized-KL / Gibbs belief, class, and kernel objectives directly, while \path{SemanticConvergence.lem_variational}, \path{SemanticConvergence.prop_two_observer_minimizer}, and \path{SemanticConvergence.prop_kernel_functional_minimizer} certify the corresponding exact Bayes/Gibbs minimizers on the paper-facing countable stack. In Section~\ref{sec:main}, the Lean declarations \path{SemanticConvergence.thm_separating_support_convergence}, \path{SemanticConvergence.thm_separating_support_rate}, and \path{SemanticConvergence.cor_separating_support_finite_time} implement the concrete probabilistic support/rate stack directly and derive it from the deterministic concrete substrate through an explicit Lean bridge, with no external bridge-equation hypothesis remaining on the public theorem surface: the first proves an almost-sure one-step residual-odds contraction on the realized trajectory law, the second upgrades it to an almost-sure positive-floor $N$-step residual-odds rate bound, and the third gives the corresponding almost-sure lower bound on the realized observer-fiber posterior share. At the boundary and surrogate end, \path{SemanticConvergence.thm_afe_near_miss}, \path{SemanticConvergence.thm_amortized_surrogate_selector_existence}, \path{SemanticConvergence.thm_amortized_surrogate}, and \path{SemanticConvergence.cor_amortized_surrogate_finite_time} now expose the repaired action-level near-miss, countable \textup{(A1)--(A3)} wrapper, deployment-side support floor, and finite-time consequence. For any interactive learner, classical or modern, the theorem reduces the question ``is this aimed at the right target?'' to a single checkable criterion: divergent cumulative policy mass on separating actions.
