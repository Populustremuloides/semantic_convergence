...
Theorem~\ref{thm:meeting-point} fixed the canonical action observer at the meeting point $\omega_A=\omega_{\mathrm{behav}}$: any observer above the meeting point caps the action bonus independent of $a$ (Proposition~\ref{prop:action-cap}), and any observer below loosens the target (Proposition~\ref{prop:goal-dialed}). What remains is to exhibit an action rule that realizes the action-side minimizer of the two-observer functional at $\omega_A=\omega_{\mathrm{behav}}$ explicitly. This section does so. The near-miss of Section~\ref{sec:near-miss} already isolated the comparison that matters: once the target is an $\omega_{\mathrm{behav}}$-class $c$, the relevant rival is not any single alternative program; it is all posterior mass outside $c$ pooled together. An action that fails to separate $c$ from that aggregate rival may be highly informative about the latent program and still supply no evidence for convergence to $c$. The \emph{class-against-complement} primitive of Definition~\ref{def:semantic-rule} minimizes the action term $-\beta\,\mathcal A_t^{\omega_{\mathrm{behav}}}[\pi;h_t,q]$ of Definition~\ref{def:two-observer-functional}; Proposition~\ref{prop:semantic-is-promotion-supporting} supplies the exploration floor the convergence theorem of Section~\ref{sec:main} requires.
