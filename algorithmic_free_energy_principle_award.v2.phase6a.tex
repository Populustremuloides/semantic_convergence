...
\begin{proposition}[Noise-channel immunity]
...
\]
Consequently, if the uncorrupted interactive setting satisfies the semantic separation condition of Definition~\ref{def:sep-condition}, the post-channel setting satisfies it with $\eta(c)$ replaced by a possibly smaller (but still positive, provided $K$ is not a constant channel on the support of $\mu_c$) constant, and the conclusion of Theorem~\ref{thm:semantic-convergence} transfers.
\end{proposition}
...
\]
Taking $-\log$ reverses the inequality: $\widetilde B_t\le B_t$. Preservation of positivity is immediate from $K$ being injective on the supports (otherwise $\mu_c$ and $\Qcal_t^{-c}$ remain distinct after the channel) together with Lemma~\ref{lem:zero-criterion}.
\end{proof}

\begin{remark}[Against entropy-bonus curiosity]
\label{rem:against-entropy-bonus}
Proposition~\ref{prop:noise-immunity} separates semantic separation from entropy-bonus objectives. Entropy-bonus curiosity rewards actions whose observations carry high marginal entropy $H(O\mid h_t,a)$; a noise channel $K$ of high entropy therefore inflates the bonus while, by Proposition~\ref{prop:noise-immunity}, leaving (or shrinking) the Bhattacharyya separation that drives posterior contraction. The class-against-complement rule of Definition~\ref{def:semantic-rule} is monotone under channel noise in the right direction: it never confuses extra noise for extra signal. This is one concrete sense in which the action-side minimizer of $\mathcal J_t^{\omega_{\mathrm{behav}}}$ differs in kind from the expected-free-energy action selector when the latter is read as an MI-plus-entropy bonus --- the decomposition used in the active-inference literature (cf.\ the MI-variant $\mathcal J_t^{\omega_B,\omega_A,\mathrm{MI}}$ after Definition~\ref{def:two-observer-functional} and the discussion of expected free energy in Remark~\ref{rem:efe-unification}).
\end{remark}

\section{Sufficient Conditions for Semantic Convergence}
\label{sec:main}

This section is the convergence payoff of the two-observer construction. Section~\ref{sec:functional} fixed the canonical action observer at $\omega_A=\omega_{\mathrm{behav}}$ (Theorem~\ref{thm:meeting-point}, Corollary~\ref{cor:canonical-pair}); Section~\ref{sec:belief} identified the belief-side minimizer of $\mathcal J_t^{\omega_{\mathrm{syn}},\omega_{\mathrm{behav}}}$ at the canonical belief observer $\omega_B=\omega_{\mathrm{syn}}$; Section~\ref{sec:semantic-action} identified the action-side minimizer at $\omega_A=\omega_{\mathrm{behav}}$ and placed the Bhattacharyya action term inside the R\'enyi-divergence family (Proposition~\ref{prop:chernoff-correspondence}). The theorem below assembles those pieces: under the universal prior (C1), Bayes/Gibbs belief (C2), the semantic separation condition (C3), and the class-against-complement rule (C4), $\mathcal J_t^{\omega_{\mathrm{behav}}}$-minimization drives the $\omega_{\mathrm{behav}}$-class posterior $q_t^\star([p^\star]\mid H_t)$ to one almost surely --- the answer the problem box of Section~\ref{sec:intro} asks for.

Section~\ref{sec:near-miss} below reads the symmetric contrast the two-observer indexing makes legible. The algorithmic free energy principle implements the \emph{top-of-lattice} specialization $\omega_A=\omega_{\mathrm{syn}}$, which Proposition~\ref{prop:action-cap} places strictly above the meeting point and therefore outside the action-admissible half-lattice; its near-miss (Theorem~\ref{thm:afe-near-miss}) and observer-promotion failure (Theorem~\ref{thm:observer-promotion-failure}) are the functional-level symptoms. The theorem of this section fills the symmetric role at the canonical choice $\omega_A=\omega_{\mathrm{behav}}$.
...
Theorem~\ref{thm:semantic-convergence} is the convergence payoff of the two-observer functional at its canonical pair $(\omega_B,\omega_A)=(\omega_{\mathrm{syn}},\omega_{\mathrm{behav}})$. Conditions (C1)--(C2) identify the belief-side minimizer of $\mathcal F_t$ in Definition~\ref{def:two-observer-functional} (Lemma~\ref{lem:variational}); (C4) is the action-side minimizer at $\omega_A=\omega_{\mathrm{behav}}$ (Definition~\ref{def:semantic-rule}); (C3) is the non-degeneracy of the Bhattacharyya action term along trajectories. Under this quadruple, $\mathcal J_t^{\omega_{\mathrm{behav}}}$-minimization delivers the answer the problem box of Section~\ref{sec:intro} asks for: almost-sure concentration on the observer-$\omega_{\mathrm{behav}}$ indistinguishability class $[p^\star]$. In particular, the theorem does \emph{not} say that $\mathcal J_t^{\omega_{\mathrm{behav}}}$ is minimized at each finite $t$; it says that the pair (Bayes/Gibbs belief, class-against-complement action) is the functional's simultaneous $(q,\pi)$-minimizer and that this pair drives $q_t^\star([p^\star]\mid H_t)\to 1$.
\end{remark}
...
\subsection{An exponential rate in expectation}
\label{sec:exp-rate}

Theorem~\ref{thm:semantic-convergence} is asymptotic; the supermartingale structure that drives it also delivers a linear-in-$t$ lower bound on the expected log-odds of the true class. Define the true-class log-odds
\[
Z_t\;:=\;\log\frac{q_t^\star(c^\star\mid H_t)}{1-q_t^\star(c^\star\mid H_t)},\qquad c^\star:=[p^\star],
\]
with the conventions $Z_t=+\infty$ when $q_t^\star(c^\star\mid H_t)=1$ and $Z_t=-\infty$ when $q_t^\star(c^\star\mid H_t)=0$. The universal prior gives $Z_0$ finite. Write $\Delta_t:=Z_{t+1}-Z_t$ and $\mathcal F_t:=\sigma(H_t,C_0,\dots,C_{t-1})$ as in the proof of Theorem~\ref{thm:semantic-convergence}.
...
\;\ge\;
2\hat\eta\,\epsilon\,\bar w(c^\star),
\qquad
...
\]
Proposition~\ref{prop:semantic-is-promotion-supporting} gives $\Prob(C_t=c^\star\mid\mathcal F_t)\ge\epsilon\bar w(c^\star)$. On $\{C_t=c^\star\}$, the semantic-rule selector satisfies $B_t(c^\star,A_{t+1};H_t)\ge\hat\eta$ by the semantic separation condition (C3), so
\[
\E[B_t(c^\star,A_{t+1};H_t)\mid\mathcal F_t]\ge\hat\eta\,\epsilon\,\bar w(c^\star).
\]
Chaining the two inequalities gives the claim.
\end{proof}

\begin{proposition}[Exponential rate, expectation form]
\label{prop:exp-rate}
Under \textup{(C1)--(C4)}, for every $t\ge 0$,
\[
\E[Z_t]\;\ge\;Z_0+\mu_0\,t,
\qquad
\mu_0\;:=\;2\hat\eta\,\epsilon\,\bar w(c^\star)\;>\;0.
\]
...
\begin{proof}
On $\{q_t^\star(c^\star\mid H_t)=1\}$, $Z_{s}=+\infty$ for every $s\ge t$ and $\Delta_s=0$, so the lower bound trivially holds. On $\{q_t^\star(c^\star\mid H_t)<1\}$, Lemma~\ref{lem:one-step-drift} gives $\E[\Delta_t\mid\mathcal F_t]\ge\mu_0$. Taking unconditional expectations, $\E[\Delta_t]\ge\mu_0$; telescoping yields $\E[Z_t]\ge Z_0+\mu_0 t$. Jensen applied to the convex function $x\mapsto e^x$ gives the exponential lower bound on the expected odds ratio.
\end{proof}

\begin{remark}[On concentration under a moment hypothesis]
\label{rem:exp-rate-concentration}
Proposition~\ref{prop:exp-rate} is the unconditional expectation-form rate. A concentration-form rate $\Prob(q_t^\star(c^\star\mid H_t)<1-e^{-ct})\le 2e^{-c't}$ follows from the same supermartingale via Azuma--Hoeffding applied to the compensated process $Z_t-\E[Z_t]$, provided one adds a moment condition $\E[\Delta_t^2\mid\mathcal F_t]\le M$ a.s. --- which is not automatic, since likelihood ratios may be extreme. A sufficient regularity is $m\le\mu_{c^\star}(o\mid h_t,a)/\Qcal_t^{-c^\star}(o\mid h_t,a)\le 1/m$ uniformly for some $m>0$. Under such a hypothesis, Proposition~\ref{prop:exp-rate} upgrades to an explicit geometric concentration around the asymptotic limit of Theorem~\ref{thm:semantic-convergence}. We state the proposition in expectation form because the unconditional bound is the form Open Question~2 of Section~\ref{sec:discussion} asks for, and the concentration form requires a hypothesis outside the problem box.
\end{remark}

\begin{corollary}[Goal-dialed convergence]
\label{cor:goal-dialed-payoff}
Fix an observer $\omega_A$ with $\omega_A\le\omega_{\mathrm{behav}}$. Under the observer-indexed conditions \textup{(C1$^{\omega_A}$)--(C4$^{\omega_A}$)} of Proposition~\ref{prop:goal-dialed},
\[
q_t^\star([p^\star]_{\omega_A}\mid H_t)\;\longrightarrow\;1
\qquad\text{almost surely,}
\]
and Proposition~\ref{prop:exp-rate} transfers with $c^\star$ replaced by $[p^\star]_{\omega_A}$, $\eta(c^\star)$ replaced by $\eta^{\omega_A}([p^\star]_{\omega_A})$, and $\bar w(c^\star)$ replaced by $\bar w^{\omega_A}([p^\star]_{\omega_A})$: the expected log-odds of $[p^\star]_{\omega_A}$ grow linearly at rate $\mu_0^{\omega_A}:=2\hat\eta^{\omega_A}\epsilon\bar w^{\omega_A}([p^\star]_{\omega_A})>0$.
\end{corollary}

\begin{proof}
Proposition~\ref{prop:goal-dialed} gives the almost-sure statement. Its proof sketch lifts Ingredients~1--5 of Theorem~\ref{thm:semantic-convergence} to $\Ccal^{\omega_A}$ in place of $\Ccal$; the one-step-drift argument of Lemma~\ref{lem:one-step-drift} uses only Lemma~\ref{lem:odds-identity} and Proposition~\ref{prop:chernoff-correspondence}, both observer-agnostic, so the telescope of Proposition~\ref{prop:exp-rate} lifts verbatim with the $\omega_A$-indexed constants.
\end{proof}

Proposition~\ref{prop:exp-rate} and Corollary~\ref{cor:goal-dialed-payoff} together read the two-observer functional as an observer dial with a quantitative rate: the paper's canonical setting $\omega_A=\omega_{\mathrm{behav}}$ targets $[p^\star]$ at rate $\mu_0$, coarser $\omega_A$'s target coarser classes at strictly larger rates $\mu_0^{\omega_A}$ (coarser classes have larger pushforward mass), and any $\omega_A>\omega_{\mathrm{behav}}$ is ruled out by Proposition~\ref{prop:action-cap}.

\section{Self-Referential Convergence: Closing the Computability Gap}
\label{sec:self-ref}
...
This section resolves the tension. A learner \emph{can} compute, at every finite $t$, the \emph{finite-time policy partition} of $\Pcal$ induced by the length-$t$ marginal of $\Prob_{p,\pi}$. Section~\ref{sec:self-ref} below shows that when the class-against-complement rule is run self-referentially --- using the finite-time policy partition generated by the rule's own prior behavior as the partition at each step --- the resulting partition chain promotes to $\omega_{\mathrm{behav}}$ almost surely under (C1)--(C3), and the convergence conclusion $q_t^\star([p^\star]\mid H_t)\to 1$ a.s.\ transfers. The formal $\omega_{\mathrm{behav}}$-targeting rule of Section~\ref{sec:main} is therefore realized by a computable approximation whose own exploration closes the gap between $\omega_\pi$ and $\omega_{\mathrm{behav}}$ along the sample path.
...
The finite-time policy observer is \emph{computable} in the sense the $\omega_{\mathrm{behav}}$-partition is not: given $(\pi,t)$ and a pair $(p_1,p_2)\in\Pcal^2$, checking whether $p_1\sim_{\omega_\pi^t}p_2$ reduces to verifying agreement of two finite-support probability distributions on histories of length $t$. This is the partition a learner can use at step $t+1$ without invoking an oracle for the full $\omega_{\mathrm{behav}}$-equivalence.
...
The inductive definition is well-posed: $\omega_{\pi^{\mathrm{sr}}}^t$ depends only on $(\pi^{\mathrm{sr}}_1,\dots,\pi^{\mathrm{sr}}_t)$ and on the length-$t$ history-space structure, both available at step $t+1$. No fixed-point computation is required. The exploration floor $\epsilon$ and the class-separating selector $a_t^c(h_t)$ are inherited from Definition~\ref{def:semantic-rule} applied at the running partition.
...
The central structural claim is that the self-referential rule's exploration floor promotes the realized policy observer all the way to $\omega_{\mathrm{behav}}$ along the sample path.

\begin{lemma}[Exploration-floor reachability]
\label{lem:exploration-reachability}
Under the self-referential rule $\pi^{\mathrm{sr}}$ of Definition~\ref{def:self-ref-rule} with exploration parameter $\epsilon\in(0,1]$, for every $t\ge 0$, every class $c\in\Ccal^{\omega_{\pi^{\mathrm{sr}}}^t}$, and every history $h_t$ with $\Prob_{p^\star,\pi^{\mathrm{sr}}}(H_t=h_t)>0$,
\[
\Prob\bigl(A_{t+1}=a_t^c(h_t)\,\big|\,H_t=h_t\bigr)\;\ge\;\epsilon\,\bar w^{\omega_{\pi^{\mathrm{sr}}}^t}(c)\;>\;0.
\]
In particular, the set of $(h_t,a_{t+1})$ pairs reachable under $\pi^{\mathrm{sr}}$ with positive probability is the support of $\Prob_{p^\star,\pi^{\mathrm{sr}}}(H_t\in\cdot)\otimes\bigcup_c\{a_t^c(h_t)\}$, the pointwise union over the running partition.
\end{lemma}

\begin{proof}
The self-referential rule applies Definition~\ref{def:semantic-rule} with $\Ccal^{\omega_{\pi^{\mathrm{sr}}}^t}$ in place of $\Ccal$, and Proposition~\ref{prop:semantic-is-promotion-supporting} transfers to any partition by the same argument: the class-targeting distribution $\nu_t$ assigns mass $\ge\epsilon\bar w^{\omega_{\pi^{\mathrm{sr}}}^t}(c)$ to each class $c$, and the conditional $\Prob(A_{t+1}=a_t^c(h_t)\mid C_t=c,H_t=h_t)=1$ by the selector construction. The universal prior of Definition~\ref{def:universal-prior} assigns positive mass to every computable program, so every class in any partition has $\bar w^\omega(c)>0$.
\end{proof}

\begin{proposition}[Observer promotion to $\omega_{\mathrm{behav}}$]
\label{prop:observer-promotion-sr}
Assume \textup{(C1)--(C3)}. Under the self-referential rule $\pi^{\mathrm{sr}}$ of Definition~\ref{def:self-ref-rule}, for every pair of programs $(p_1,p_2)\in\Pcal^2$ with $p_1\not\sim_{\mathrm{behav}}p_2$, there exists a finite random time $T_{p_1,p_2}$ (almost surely finite with respect to $\Prob_{p^\star,\pi^{\mathrm{sr}}}$) such that for every $t\ge T_{p_1,p_2}$,
\[
p_1\not\sim_{\omega_{\pi^{\mathrm{sr}}}^t}p_2.
\]
Consequently, along $\Prob_{p^\star,\pi^{\mathrm{sr}}}$-almost every sample path,
\[
...
\begin{proof}
Fix $(p_1,p_2)$ with $p_1\not\sim_{\mathrm{behav}}p_2$: there exists $(h^{\circ},a^{\circ})$ with $\mu_{p_1}(\cdot\mid h^{\circ},a^{\circ})\neq\mu_{p_2}(\cdot\mid h^{\circ},a^{\circ})$. Let $t^\circ:=|h^\circ|$ be the length of the separating history. By (C3), there is some class $c^\circ\in\Ccal^{\omega_{\pi^{\mathrm{sr}}}^{t^\circ}}$ (containing exactly one of $p_1,p_2$ since they are already $\omega_{\mathrm{syn}}$-distinct) whose class-against-complement selector $a_{t^\circ}^{c^\circ}(h^\circ)=a^\circ$. (If $a^\circ$ is not literally the selector, a separating action for $c^\circ$ at $h^\circ$ exists by (C3) and distinguishes $\mu_{p_1}$ from $\mu_{p_2}$ at some observation, which suffices for the argument that follows.)

By Lemma~\ref{lem:exploration-reachability}, $\Prob_{p^\star,\pi^{\mathrm{sr}}}(H_{t^\circ}=h^\circ,A_{t^\circ+1}=a^\circ)\ge\kappa$ for some $\kappa>0$ depending on $(p_1,p_2)$ but not on further history. The event $E_s:=\{H_s=h^\circ, A_{s+1}=a^\circ\text{ for infinitely many }s\ge s_0\}$ has probability one by the conditional Borel--Cantelli lemma applied to the $\sigma$-algebra $\mathcal F_s$ and the lower bound $\Prob(H_s\in\cdot\mid\mathcal F_{s-1})\ge\kappa^{1_{H_{s-1}=h^\circ}}$ along the sample path (the history $h^\circ$ is reachable and the policy's mixing condition under the exploration floor ensures return infinitely often, in the infinite product space).

Under an infinitely-repeated play of $a^\circ$ at $h^\circ$, the length-$s$ marginals $\Prob_{p_1,\pi^{\mathrm{sr}}}(H_s\in\cdot)$ and $\Prob_{p_2,\pi^{\mathrm{sr}}}(H_s\in\cdot)$ diverge: the post-$a^\circ$ observation distributions differ, so the probability of any fixed observation trajectory under $p_1$ differs from that under $p_2$ whenever the trajectory passes through $(h^\circ,a^\circ)$. Hence there is a minimal $T_{p_1,p_2}<\infty$ a.s.\ at which the length-$T$ marginals first differ, yielding $p_1\not\sim_{\omega_{\pi^{\mathrm{sr}}}^{T_{p_1,p_2}}}p_2$. Monotonicity (Lemma~\ref{lem:monotone-refinement}) extends the separation to every subsequent $t$.

For the second statement: the set of pairs $\{(p_1,p_2)\in\Pcal^2:p_1\not\sim_{\mathrm{behav}}p_2\}$ is countable, since $\Pcal$ is countable. By countable additivity applied to the almost-sure finiteness of each $T_{p_1,p_2}$, all such pairs are simultaneously separated by some finite $t$ almost surely, so the $\omega_{\pi^{\mathrm{sr}}}^t$-partitions refine to the $\omega_{\mathrm{behav}}$-partition in the limit a.s.\ Equivalently, $\omega_{\pi^{\mathrm{sr}}}=\omega_{\mathrm{behav}}$ on the sample path, and $R_{\pi^{\mathrm{sr}}}(p^\star)=[p^\star]_{\omega_{\pi^{\mathrm{sr}}}}=[p^\star]_{\mathrm{behav}}=[p^\star]$.
\end{proof}

Proposition~\ref{prop:observer-promotion-sr} is the structural content behind the ``stone-turning'' framing: the self-referential rule's own exploration floor supplies the separating actions that promote the policy observer to the behavioral one, which is the lifted form of what the class-against-complement primitive does at every step. The rule does not need an oracle for $\sim_{\mathrm{behav}}$; its running record of $\Prob_{p,\pi^{\mathrm{sr}}}(H_t\in\cdot)$ realizes it in the limit.

\subsection{Self-referential convergence}
\label{sec:self-ref-convergence}

\begin{theorem}[Self-referential convergence]
\label{thm:self-ref-convergence}
Assume \textup{(C1)--(C3)} and $\omega_B=\omega_{\mathrm{syn}}$ with Bayes/Gibbs belief \textup{(C2)}. Under the self-referential semantic rule $\pi^{\mathrm{sr}}$ of Definition~\ref{def:self-ref-rule},
\[
q_t^\star([p^\star]\mid H_t)\;\longrightarrow\;1
\qquad\text{almost surely.}
\]
...
\begin{proof}[Proof sketch]
Fix $p'\in\Pcal\setminus[p^\star]$, so $p'\not\sim_{\mathrm{behav}}p^\star$. By Proposition~\ref{prop:observer-promotion-sr}, there is a finite random time $T_{p'}:=T_{p^\star,p'}$ after which $p'$ and $p^\star$ lie in distinct $\omega_{\pi^{\mathrm{sr}}}^t$-classes. From time $T_{p'}$ onward, the class-against-complement selector for the class containing $p^\star$ plays an action $a$ with $B_t^{\omega_{\pi^{\mathrm{sr}}}^t}(c^\star_t,a;H_t)\ge\tfrac{1}{2}\eta(c^\star_t)$ where $c^\star_t:=[p^\star]_{\omega_{\pi^{\mathrm{sr}}}^t}$, and Lemma~\ref{lem:exploration-reachability} gives $\Prob(A_{t+1}=a\mid H_t)\ge\epsilon\bar w^{\omega_{\pi^{\mathrm{sr}}}^t}(c^\star_t)>0$. The one-step drift argument of Lemma~\ref{lem:one-step-drift} applied at the running partition gives $\E[\log(q_t^\star(p^\star\mid H_t)/q_t^\star(p'\mid H_t))\mid\mathcal F_t]\ge$ (strictly positive constant depending on $p'$) from time $T_{p'}$ onward. Hence $q_t^\star(p'\mid H_t)/q_t^\star(p^\star\mid H_t)\to 0$ a.s.

The set $\Pcal\setminus[p^\star]$ is countable; by dominated convergence applied to the posterior (viewed as a probability measure on $\Pcal$), $\sum_{p'\in\Pcal\setminus[p^\star]}q_t^\star(p'\mid H_t)\to 0$ a.s., where the dominating envelope is the constant $1$ and the pointwise a.s.\ convergence is per-pair. Equivalently, $q_t^\star(\Pcal\setminus[p^\star]\mid H_t)\to 0$ a.s., i.e., $q_t^\star([p^\star]\mid H_t)\to 1$ a.s.

The argument transfers the ingredients of Theorem~\ref{thm:semantic-convergence} onto a time-inhomogeneous partition: Ingredients~1--2 (positivity of the pushforward prior; separation) hold at every partition by (C1)--(C3); Ingredients~3--4 (Borel--Cantelli and Bhattacharyya contraction) are observer-agnostic; and Ingredient~5 (promotion support) is supplied by Lemma~\ref{lem:exploration-reachability}. The additional step is the monotone-refinement closure of Proposition~\ref{prop:observer-promotion-sr}, which replaces the $\omega_{\mathrm{behav}}$-partition oracle of Definition~\ref{def:semantic-rule}.
\end{proof}

\begin{remark}[Computability of the self-referential rule]
\label{rem:self-ref-computability}
At every finite step $t$, the self-referential rule requires only the finite data $(\pi^{\mathrm{sr}}_1,\dots,\pi^{\mathrm{sr}}_t)$ and the length-$t$ marginal $\Prob_{p,\pi^{\mathrm{sr}}}(H_t\in\cdot)$ for each $p$ the learner's posterior assigns positive mass to, to compute the $\omega_{\pi^{\mathrm{sr}}}^t$-partition. Both objects are semicomputable under the universal prior: the length-$t$ marginal is a finite sum of products of one-step transition probabilities, and the posterior's support at time $t$ is $\{p:\mu_p(o_{1:t}\mid a_{1:t})>0\}$, semicomputable by Solomonoff's lemma. Theorem~\ref{thm:self-ref-convergence} therefore closes the computability gap left open by Theorem~\ref{thm:semantic-convergence}: a learner with access to the universal prior and the realized transcript can approximate the $\omega_{\mathrm{behav}}$-partition at every step to the precision required for convergence, without invoking an oracle for semantic equivalence.
\end{remark}

\begin{remark}[Relation to Proposition~\ref{prop:goal-dialed}]
\label{rem:self-ref-vs-goal-dialed}
Proposition~\ref{prop:goal-dialed} is a \emph{fixed-observer} convergence statement: for any single $\omega_A\le\omega_{\mathrm{behav}}$, the rule at $\omega_A$ drives the posterior onto $[p^\star]_{\omega_A}$. Theorem~\ref{thm:self-ref-convergence} is a \emph{time-inhomogeneous} statement: the running observer $\omega_{\pi^{\mathrm{sr}}}^t$ refines with $t$, and the payoff is convergence to $[p^\star]$ at the meeting-point observer $\omega_{\mathrm{behav}}$ itself. The two results do not collide: Proposition~\ref{prop:goal-dialed} treats the observer dial as a coarsening knob for deliberate targeting at $\omega_A\le\omega_{\mathrm{behav}}$; Theorem~\ref{thm:self-ref-convergence} answers the orthogonal question of whether the formally-specified $\omega_{\mathrm{behav}}$-rule admits a computable approximation that recovers the same conclusion.
\end{remark}

The self-referential rule turns a specification into a procedure. The observer-first reading of the paper's main theorem (Theorem~\ref{thm:semantic-convergence}) asks whether a learner can recover the behavioral class of the environment; the self-referential rule's partition promotion (Proposition~\ref{prop:observer-promotion-sr}) supplies the concrete algorithmic realization, and Theorem~\ref{thm:self-ref-convergence} confirms that the answer under (C1)--(C3) is the same whether the rule targets the behavioral partition exactly or the rule's own running finite-time approximation to it.

\section{The Algorithmic Free Energy Principle as Boundary Example}
\label{sec:boundary-example}
\label{sec:near-miss}

Section~\ref{sec:main} identified the minimizer of $\mathcal J_t^{\omega_{\mathrm{behav}}}$ at $\beta,\gamma>0$ --- the Bayes/Gibbs posterior paired with the class-against-complement rule --- as the triple the problem box of Section~\ref{sec:intro} asks for, and proved almost-sure concentration on $[p^\star]$. This section turns to the boundary of the admissible range on the action side. The \emph{algorithmic free energy principle} --- Bayes/Gibbs belief under the universal prior paired with expected-free-energy action --- is the minimizer of the mutual-information variant of the two-observer functional at action observer $\omega_A=\omega_{\mathrm{syn}}$, a setting strictly above the meeting point of Theorem~\ref{thm:meeting-point}. The same Bayes/Gibbs architecture that delivered the belief-side unification of Section~\ref{sec:belief} therefore corresponds to a \emph{wrong-observer} instance of the two-observer functional: the action bonus is the syntactic-class mutual information rather than the behavioral-class Bhattacharyya separation, and by Proposition~\ref{prop:action-cap} this bonus is capped on behavioral twins. Theorem~\ref{thm:afe-near-miss} below exhibits the structural failure of this instance, and Theorem~\ref{thm:observer-promotion-failure} reads it at the observer level. Interpreted through the two-observer functional, the AFE principle is not a competitor of the main theorem; it is the boundary example that fixes where the admissible range ends.
...
The algorithmic free energy principle of Definition~\ref{def:afe-principle} is the minimizer of the action-MI variant $\mathcal J_t^{\omega_{\mathrm{syn}},\mathrm{MI}}$ at $\beta=1$: its belief is the unique $q$-minimizer of the belief term $\Fcal_t$ (Lemma~\ref{lem:variational}) and, with zero action cost and uniform preferences on a common finite feasible observation set, its action rule selects $a_{t+1}\in\arg\max_a \I_{q_t^\star}(P;O\mid h_t,a)$, which is the $\pi$-minimizer of $-\I(C^{\omega_A};O\mid h_t,a)$ with $C^{\omega_A}=P$ at $\omega_A=\omega_{\mathrm{syn}}$. Since $\omega_{\mathrm{syn}}>\omega_{\mathrm{behav}}$, this action observer lies strictly above the meeting point of Theorem~\ref{thm:meeting-point}, and Proposition~\ref{prop:action-cap} caps the action bonus on every pair of behavioral twins. Conversely, the minimizer of $\mathcal J_t^{\omega_{\mathrm{behav}}}$ at $\beta,\gamma>0$ is the (Bayes/Gibbs posterior, class-against-complement rule) pair of Section~\ref{sec:main}, which under \textup{(C1)--(C3)} achieves $q_t^\star([p^\star]\mid H_t)\to 1$ almost surely (Theorem~\ref{thm:semantic-convergence}).
\end{proposition}
...
\begin{remark}[The AFE principle as $\mathcal J_t^{\omega_{\mathrm{syn}},\mathrm{MI}}$-minimizing agent]
\label{rem:afe-as-functional-minimizer}
Under the two-observer functional, Proposition~\ref{prop:boundary-identity} identifies Definition~\ref{def:afe-principle}'s triple as the minimizer of $\mathcal J_t^{\omega_{\mathrm{syn}},\mathrm{MI}}$. Item (i) is the universal prior that fixes the reference measure of $\mathcal J_t$; item (ii) is the $q$-minimization of the belief term $\Fcal_t$; item (iii) is the $\pi$-minimization of the action-MI term at $\omega_A=\omega_{\mathrm{syn}}$. The principle therefore has a two-word synopsis in this paper's vocabulary: \emph{$\mathcal J_t^{\omega_{\mathrm{syn}},\mathrm{MI}}$-minimizing agent}. Theorem~\ref{thm:afe-near-miss} below is the proof that this minimizer fails to concentrate on $[p^\star]$.
\end{remark}
...
The near-miss isolates the structural failure of the AFE minimizer. Sharpening the construction makes the contrast with the main theorem's promotion-supporting rule quantitative on a single learning problem: the two rules are run against the same prior, and the witness $p'$ of Theorem~\ref{thm:observer-promotion-failure} is put on the support.
...
Second, what quantitative rates can be proved? Theorem~\ref{thm:semantic-convergence} is asymptotic. Proposition~\ref{prop:exp-rate} delivers an unconditional linear-in-$t$ lower bound on the expected true-class log-odds, partially closing this question; Remark~\ref{rem:exp-rate-concentration} indicates that a geometric concentration upgrade is available under a likelihood-ratio regularity hypothesis. What remains open is a finite-horizon lower bound on $q_t^\star([p^\star]\mid h_t)$ itself (as opposed to its expectation) in the absence of such a regularity hypothesis. Sequential design results in statistics suggest such bounds should be possible in restricted regimes \citep{Chernoff1959,BoxHill1967}.
