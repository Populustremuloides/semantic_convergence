...
\section{Universal Bayesian Belief}
\label{sec:belief}

The belief problem asks for a posterior over computable interactive environments that is well behaved enough to support any subsequent convergence theorem. Answering it requires an object that couples the universal prior of Solomonoff and Hutter with the variational form of active inference. This section formulates that object --- the \emph{algorithmic free energy} --- and proves that, under the universal prior, its minimizer is the exact Bayes/Gibbs posterior.
...
\subsection{Algorithmic free energy and the Bayes/Gibbs variational identity}

For $p\in\Pcal$ and history $h_t$, define the cumulative history-fit loss
\[
L_t(p;h_t):=-\log\mu_p(o_{1:t}\givena a_{1:t}),
\]
with the convention $L_t=+\infty$ if the likelihood vanishes.
...
\begin{remark}
Predictive merging places the Bayes/Gibbs posterior, in a one-step predictive sense, inside the compatibility set $R_{h_t}(p^\star)$: the posterior predictive law matches the true next-step law along the realized interaction. This is the Solomonoff-interactive analogue of the classical merging-of-opinions phenomenon \citep{BlackwellDubins1962}. The gap between $R_{h_t}(p^\star)$ and $[p^\star]$ is therefore not closed by belief alone.
\end{remark}
