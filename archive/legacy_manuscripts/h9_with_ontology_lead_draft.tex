...
\large H7 Lean-Verified Interface Draft}
\author{}
\date{}
...
\begin{center}
\fbox{\parbox{0.93\textwidth}{\textbf{H7 status.}
This is a Lean-facing verification copy of the manuscript. The original
manuscript source \path{semantic_convergence_interactive_learning.tex} is left
unchanged as the more ambitious inspirational draft. In this copy, the positive
semantic-convergence claim is stated at the route-2 verified kernel-functional
interface. The main theorem uses one formal assumptions package
(Assumptions~\ref{ass:main-verified-package}) for the three objects that must
match: the realized learning instance $\mathcal I$, the minimized
kernel-functional $\mathcal J$, and the semantic target set $\Scal(p^\star)$.
The Lean development now derives the semantic calibration/coherence bridge
internally from the canonical predictive-kernel observer. The remaining
load-bearing premises are the explicit normalizer, prefix-Hellinger, and
class-predictive separation assumptions listed there.}}
\end{center}

\begin{abstract}
Three layers govern any interactive learning question. An \emph{observer} is a passive lens: a measurable map from programs to views that fixes what is in principle distinguishable through a given access pattern. An \emph{agent} is an active learning architecture --- prior, belief-update rule, action rule, preferences --- that operates in isolation from any specific environment. A \emph{coupling} is a joint property of an observer, an agent, and an environment that says whether the agent's actual operation, viewed through the observer, reaches the observer's ceiling. Section~\ref{sec:ontology} states the ontology formally; Parts~\ref{part:observer}--\ref{part:coupling} develop each layer in turn.

\medskip
\noindent
At the observer layer, four nested sets $\{p^\star\}\subseteq[p^\star]\subseteq R_\pi(p^\star)\subseteq R_{h_t}(p^\star)$ collect the indistinguishability classes of four observers at increasing observational power. The strict-hierarchy theorem rules out reductions, and the observable-quotient theorem identifies the \emph{interventional semantic class} $[p^\star]$ as the ceiling for any history-based learner: any target a history-based learner can reliably recover is constant on $[p^\star]$. At the agent layer, the \emph{algorithmic free energy} --- the Gibbs-form variational functional whose reference measure is the Kolmogorov-complexity-weighted universal interactive prior of \citet{Solomonoff1964a,Solomonoff1964b,Kolmogorov1965,Hutter2005} rather than a stipulated parametric prior --- unifies three traditions on both sides of the inference problem: its belief-side minimization is exactly Bayesian updating with the KL divergence forced by convex duality, and its zero-cost uniform-preference action term recovers the information-gain criterion shared by Bayesian experimental design, compression-based curiosity, and active-inference curiosity \citep{Friston2010,Lindley1956,ChalonerVerdinelli1995,MacKay1992,OrtegaBraun2013,Schmidhuber1991,Schmidhuber2010}. These layers are environment-independent.

\medskip
\noindent
The central question is at the coupling layer. The Bayes/Gibbs/expected-free-energy architecture can fail: a substituted finite-support prior freezes the posterior on $[p^\star]$ at its prior mass for every finite horizon. The verified positive theorem of this H7 copy answers the boxed problem at the coupling layer. When an observer-agent-environment triple satisfies a small set of explicit coupling conditions --- exact Bayes/Gibbs belief, exact kernel-functional action against the canonical predictive-kernel observer, well-formed target-fiber normalizers, the true-law prefix-Hellinger obligations, and class-predictive Bhattacharyya separation on the reference support --- the posterior converges almost surely to the observer's interventional class for the environment. The semantic-to-true-law calibration step is internal to the theorem for the canonical predictive-kernel observer, so raw semantic coherence is no longer a separate coupling assumption. The older support/rate route is retained as a stronger zero-out special-case companion. The diagnostic vocabulary of Section~\ref{sec:ontology-diagnostic} sorts coupling failures into wrong-observer geometry, structurally inadequate agent, and insufficient separating support; each named theorem in this paper is the canonical exhibit for one diagnosis.

\medskip
\noindent
A public Lean 4 repository at \url{https://github.com/Populustremuloides/semantic_convergence} contains Lean counterparts for the manuscript items over the explicit concrete foundation used in the repo, together with generated coverage, proof-shape, exactness-lock, and per-declaration axiom audits. At the agent layer, \path{SemanticConvergence.def_afe}, \path{SemanticConvergence.def_two_observer_functional}, and \path{SemanticConvergence.def_kernel_functional} implement the repaired Gibbs objectives, while \path{SemanticConvergence.lem_variational}, \path{SemanticConvergence.lem_kl_necessity}, \path{SemanticConvergence.prop_two_observer_minimizer}, and \path{SemanticConvergence.prop_kernel_functional_minimizer} certify the corresponding exact variational identities and minimizer theorems. At the coupling layer, the verified main theorem is \path{SemanticConvergence.thm_constructed_infinite_bayes_gibbs_trueLaw_hellinger_spine_convergence_of_constrained_kernel_functional_minimizers_classPredictive_predictiveKernelObserver}; its bridge uses \path{SemanticConvergence.countablePredictiveKernelObserver} and the \path{..._predictiveKernelObserver} semantic-calibration lemmas to remove raw coherence assumptions. The concrete support/rate declarations \path{SemanticConvergence.thm_separating_support_convergence}, \path{SemanticConvergence.thm_separating_support_rate}, and \path{SemanticConvergence.cor_separating_support_finite_time} remain as a stronger zero-out special-case route with its own update/support-separation exactness locks.
\end{abstract}
...
This H7 copy answers the boxed problem through the exact kernel-functional route. The answer is packaged as a matched triple $\mathfrak K=(\mathcal I,\mathcal J,\Scal)$: $\mathcal I$ is the realized Bayes/Gibbs learning instance, $\mathcal J$ is the kernel functional minimized at each history, and $\Scal(p^\star)$ is the predictive semantic fiber whose posterior mass should converge to one. Theorem~\ref{thm:separating-support-convergence} says that when this triple satisfies the single formal package Assumptions~\ref{ass:main-verified-package}, exact Bayes/Gibbs belief plus exact kernel-functional minimization drives the posterior onto $\Scal(p^\star)$. The key upgrade over the previous interface is that the semantic-to-true-law calibration step is no longer a free-standing coherence assumption: the canonical predictive-kernel observer makes it internal to the theorem. The older support/rate route is retained below as a concrete zero-out special case and as a companion to the main route; its sharpness results still explain the two failure modes, wrong-observer geometry and insufficient separating support.

Section~\ref{sec:setup} fixes the interactive setting. Section~\ref{sec:ontology} states the three-layer ontology --- observer, agent, coupling --- that organizes the rest of the paper. Part~\ref{part:observer} (Section~\ref{sec:hierarchy}) is the observer side: it constructs the four-set hierarchy, proves the observable-quotient theorem, and witnesses the three strict inclusions. Part~\ref{part:agent} (Sections~\ref{sec:functional}--\ref{sec:belief}) is the agent side: Section~\ref{sec:functional} introduces the two-observer functional $\mathcal J_t^{\omega_B,\omega_A}$ together with its exact action-realizing kernel lift $\mathcal J_{t,\mathrm{ker}}^{\omega_B,\omega_A}$ and proves the meeting-point theorem; Section~\ref{sec:belief} identifies the belief-side minimizer at the canonical belief observer and proves the variational and KL-necessity identities. Part~\ref{part:coupling} (Sections~\ref{sec:semantic-action}--\ref{sec:near-miss}) is the coupling layer: Section~\ref{sec:semantic-action} identifies the exact kernel action law and the canonical class-against-complement selector realization at the canonical action observer, and proves separation transfer through decodable observation channels; Section~\ref{sec:main} states the consolidated H7 coupling conditions on the package $\mathfrak K=(\mathcal I,\mathcal J,\Scal)$ and proves the verified main theorem from them, then keeps the stronger support/rate stack as a conditional zero-out special-case route; Section~\ref{sec:self-ref} replaces the uncomputable behavioral observer by a finite-time proxy; Section~\ref{sec:near-miss} reads the algorithmic free energy principle as the boundary example outside the action-admissible range and proves the near-miss. Section~\ref{sec:implications} records a differentiable surrogate loss (Section~\ref{sec:amortized-surrogate}) whose deployment-side guarantee is conditional on the same support/spine interface. Sections~\ref{sec:discussion} and~\ref{sec:conclusion} draw consequences.
...
Section~\ref{sec:main} establishes the verified \emph{convergence payoff}: the route-2 kernel-functional theorem proves posterior concentration from exact Bayes/Gibbs belief, exact kernel-functional minimization, the canonical predictive-kernel observer, and the consolidated Assumptions~\ref{ass:main-verified-package}. The older support theorem, rate theorem, selector-free kernel theorem, canonical selector theorem, compact-action extension, and matched converses are retained as the conditional support-route program and zero-out companion stack.
...
\item[(ii)] Suppose the learner uses the same universal-prior weights $w(p)$ on the common program index set, the Bayes/Gibbs posterior $\widetilde q_t$ on corrupted histories, and the semantic action rule of Definition~\ref{def:semantic-rule} computed from the transformed family $(\widetilde\mu_p)_{p\in\Pcal}$. If the induced corrupted process satisfies the Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}, then
\[
...
\]
where $\pi^{\mathrm{sem}}$ is the semantic-rule action marginal on corrupted histories. Applying Theorem~\ref{thm:separating-support-convergence} to the corrupted family, together with the Hellinger-spine hypothesis in part \textup{(ii)}, yields part \textup{(ii)}.
...
\qquad\text{almost surely on }\{\widetilde q_t(c^\star\mid \widetilde H_t)<1\},
\]
where $\pi^{\mathrm{ker}}$ is the action marginal of the corrupted kernel rule. Applying Theorem~\ref{thm:separating-support-convergence} to the corrupted family, together with the Hellinger-spine hypothesis in part \textup{(iii)}, yields part \textup{(iii)}.
...
\item[(iii)] if the learner uses the same prior weights on program indices, Bayes/Gibbs belief for the corrupted family, either the semantic action rule or the kernel semantic action rule computed from $(\widetilde\mu_p)_{p\in\Pcal}$, and the corresponding Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}, then the corresponding conclusions of Theorems~\ref{thm:semantic-convergence} and~\ref{thm:kernel-semantic-convergence} hold with $\eta(c)$ replaced by $\widetilde\eta_K(\eta(c))$;
...
\qquad\text{almost surely on }\{\widetilde q_t(c^\star\mid H_t)<1\}.
\]
Applying Theorem~\ref{thm:separating-support-convergence} together with the corresponding Hellinger-spine hypothesis yields the kernel conclusion.
...
Section~\ref{sec:functional} fixed the canonical action observer at $\omega_A=\omega_{\mathrm{behav}}$ (Corollary~\ref{cor:canonical-pair}) and Section~\ref{sec:belief} fixed the canonical belief observer at $\omega_B=\omega_{\mathrm{syn}}$; this section states the verified convergence interface for what happens once the learner acts there. The H7 main theorem uses the exact kernel-functional action rule and the canonical predictive-kernel observer. Its natural object is not the target program by itself but a semantic-learning package $\mathfrak K=(\mathcal I,\mathcal J,\Scal)$: the realized learning instance, the functional minimized by the action rule, and the semantic set whose posterior mass is supposed to converge. Assumptions~\ref{ass:main-verified-package} is the single load-bearing package for that theorem: it lists the substrate, Bayes/Gibbs belief, exact minimization, normalizer, prefix-Hellinger, and class-predictive separation hypotheses in one place.

The rest of the section retains the original support/rate program as a conditional companion and as a map of the stronger zero-out route. Statements that apply the older support theorem should be read in this H7 copy through Assumptions~\ref{ass:main-verified-package} when they are part of the verified main route, or as belonging to the separate concrete support/rate stack tracked in the Lean manifest.
...
\]
with the following components. The instance $\mathcal I$ contains the environment family, universal prior, true program, policy, reference action laws, inverse-temperature parameters, observers, posterior process, and infinite trajectory law. The functional $\mathcal J$ is the history-indexed action objective actually minimized by the learner; in the H7 route it is
\[
\mathcal J
...
\begin{assumptions}[H7-verified semantic-learning package]
...
\]
The package is H7-verified when:
\begin{itemize}
...
\begin{proof}
This is the manuscript counterpart of the Lean declaration \path{SemanticConvergence.thm_constructed_infinite_bayes_gibbs_trueLaw_hellinger_spine_convergence_of_constrained_kernel_functional_minimizers_classPredictive_predictiveKernelObserver}.
...
Fix a policy $\pi$, a target program $p^\star\in\Pcal$, and write $c^\star:=[p^\star]$. Work on the infinite realized-trajectory law of the Bayes/Gibbs process. Suppose there is a constant $\kappa>0$ such that:
\begin{itemize}
\item[(i)] every realized action is policy-supported at its realized prefix, almost surely;
\item[(ii)] for every realized prefix $h_t$ and every action $a$ in the support of $\pi_{t+1}(\cdot\mid h_t)$, the observer-fiber class/complement predictive Bhattacharyya affinity satisfies
\[
\sum_{o\in\Ocal}
\sqrt{
\mu_{c^\star}(o\mid h_t,a)\,
\Qcal_t^{-c^\star}(o\mid h_t,a)
}
\le \exp(-\kappa);
\]
\item[(iii)] the Bayes/Gibbs Hellinger envelope
\[
M_t
:=
\exp\!\left(\sum_{i=0}^{t-1}B_i(c^\star,A_{i+1};H_i)\right)
\sqrt{
\frac{1-q_t^\star(c^\star\mid H_t)}
     {q_t^\star(c^\star\mid H_t)}
}
\]
is an $L^1$-bounded martingale and satisfies the square-root Bayes identity of Definition~\ref{def:hellinger-spine}.
\end{itemize}
Then
\[
...
\begin{proof}
By (i) and (ii), every realized one-step Bhattacharyya separation is at least $\kappa$ almost surely:
\[
B_t(c^\star,A_{t+1};H_t)\ge \kappa
\qquad
\text{for every }t.
\]
Thus the cumulative separation
\[
S_t:=\sum_{i=0}^{t-1}B_i(c^\star,A_{i+1};H_i)
\]
dominates $\kappa t$ and hence diverges to $+\infty$ almost surely. Assumption (iii) supplies the martingale, $L^1$ bound, and square-root identity legs of Definition~\ref{def:hellinger-spine}; residual nonnegativity is immediate from the definition of posterior odds. Therefore the Hellinger convergence spine holds, and Theorem~\ref{thm:separating-support-convergence} gives the claim.
\end{proof}

\begin{remark}[H7 boundary for the support route]
\label{rem:h6-support-boundary}
The original manuscript's support theorem used a weaker stochastic hypothesis:
divergent cumulative policy mass on a set of separating actions. The H7 main
route instead uses the exact kernel-functional minimizer together with the
canonical predictive-kernel observer and the consolidated assumptions above.
...
main route, or as belonging to the separate zero-out special-case route tracked
in the Lean manifest.
\end{remark}
...
\text{for every }t\ge 0,\ h_t,\ a\in\Acal.
\]
Assume also that, for each target $p^\star$, the Bayes/Gibbs residual process induced by this policy satisfies the Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}.
...
\]
and the assumed Hellinger spine gives the posterior-concentration claim.
\end{proof}
...
\item[(C5)] for the induced infinite realized process and each target class, the Bayes/Gibbs residual odds satisfy the Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}.
\end{itemize}
Then for every $p^\star\in\Pcal$,
\[
q_t^\star([p^\star]\mid H_t)\longrightarrow 1
...
\item[(K5)] for the induced infinite realized process and each target class, the Bayes/Gibbs residual odds satisfy the Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}.
\end{itemize}
Then for every $p^\star\in\Pcal$,
\[
q_t^\star([p^\star]\mid H_t)\longrightarrow 1
...
(C_t,A_{t+1})\sim \kappa_{t,\mathrm{cts}}^{\mathrm G,\omega_{\mathrm{behav}}}(\cdot,\cdot\mid H_t).
\]
\item[(v)] for the induced infinite realized process, the Bayes/Gibbs residual odds satisfy the Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}.
\end{itemize}
...
\]
and the assumed Hellinger spine gives part \textup{(a)}. Under the bounded-ratio hypothesis, applying Theorem~\ref{thm:separating-support-rate} with the same support floor and with $\eta(c^\star)$ replaced by $\eta_\lambda(c^\star)$ gives part \textup{(b)}, since
\[
2\,\eta_\lambda(c^\star)\,\delta_0\,\bar w(c^\star)\,\rho_\lambda(c^\star)
=
\eta(c^\star)\,\delta_0\,\bar w(c^\star)\,\rho_\lambda(c^\star)
...
Fix a target program $p^\star\in\Pcal$ and write $c^\star:=[p^\star]$. Under (C1), (C2), the Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}, and a finite semantic class set $\Ccal$ with $\eta>0$ such that whenever $q_t^\star(c^\star\mid H_t)<1$, some action $a$ satisfies $B_t(c,a;H_t)\ge\eta$ for every unresolved class $c$, define
\[
U_t(a;H_t):=
\begin{cases}
...
\qquad\text{almost surely on }\{q_t^\star(c^\star\mid H_t)<1\}.
\]
Applying Theorem~\ref{thm:separating-support-convergence} with $r_t\equiv 1$ gives the support-route claim once the corresponding Hellinger spine is supplied.
\end{proof}
...
\]
\item[(A4)] under the deployed policy $\pi^{\mathrm{impl}}$, the Bayes/Gibbs residual odds for $c^\star$ satisfy the Hellinger-spine hypothesis of Theorem~\ref{thm:separating-support-convergence}.
\end{itemize}
...
\pi_{t+1}^{\mathrm{impl}}(S_t^\star\mid h_t)\ge \delta_{\mathrm{impl}}
\]
for every reachable $h_t$. Applying Theorem~\ref{thm:separating-support-convergence} with $r_t\equiv \delta_{\mathrm{impl}}$ and the assumed Hellinger spine gives
\[
...
The boxed problem has two logically distinct parts. The belief rule is fully characterized: the Bayes/Gibbs posterior under the universal prior is the unique minimizer of the algorithmic free energy (Lemma~\ref{lem:variational}), and within a natural convex divergence class the KL-based functional is the only one with that property (Lemma~\ref{lem:kl-necessity}). The action/convergence rule is verified in Theorem~\ref{thm:separating-support-convergence}: a matched semantic-learning package $\mathfrak K=(\mathcal I,\mathcal J,\Scal)$, interpreted through the canonical predictive-kernel observer and the consolidated Assumptions~\ref{ass:main-verified-package}, yields almost-sure posterior concentration on $\Scal(p^\star)$. The stronger original support route remains a companion: Theorem~\ref{thm:afe-near-miss} fails by using the wrong observer geometry, while Theorems~\ref{cor:support-necessary} and~\ref{thm:summable-support-insufficient} fail in a different register --- the action rule targets the right observer, but the realized trajectory withholds enough mass from genuinely separating actions. The action objective supporting semantic recovery must therefore be class-targeted and sufficiently supported along the realized trajectory.
...
The paper develops three layers in turn. At the \emph{observer} layer, the four nested sets $\{p^\star\}\subseteq[p^\star]\subseteq R_\pi(p^\star)\subseteq R_{h_t}(p^\star)$ are the indistinguishability classes of four observers at increasing observational power, with all three inclusions strict in general; the observable-quotient theorem identifies $[p^\star]$ as the ceiling for any history-based learner. At the \emph{agent} layer, the universal prior and Bayes/Gibbs posterior minimize the algorithmic free energy, and at zero action cost with uniform preferences the resulting expected free energy is the information-gain criterion shared by Bayesian experimental design, compression-based curiosity, and active-inference curiosity; the architecture is a structurally well-formed agent. At the \emph{coupling} layer, the same architecture can fail: the near-miss theorem (Theorems~\ref{thm:afe-near-miss} and~\ref{thm:observer-promotion-failure}) exhibits a substituted-prior coupling on which the realized policy induces an observer strictly coarser than the behavioral one, so the posterior on $[p^\star]$ does not concentrate. Theorem~\ref{thm:separating-support-convergence} is the verified positive answer at the coupling layer: when the instance $\mathcal I$, minimized functional $\mathcal J$, and target set $\Scal(p^\star)$ form an H7-verified semantic-learning package --- equivalently, when an admissible agent is $\kappa$-coupled to the environment through the canonical predictive-kernel observer --- the Bayes/Gibbs posterior converges almost surely to $\Scal(p^\star)$. The older support/rate arguments are retained as a stronger zero-out special-case companion; in the original support geometry, the condition is sharp --- zero separating support makes uniform recovery impossible (Corollary~\ref{cor:support-necessary}) and summable separating support still admits failure with positive probability (Theorem~\ref{thm:summable-support-insufficient}). The diagnostic vocabulary of Section~\ref{sec:ontology-diagnostic} sorts these failure modes into observer failure, agent failure, and coupling failure, and assigns each named theorem to one of those diagnoses.
...
The public Lean 4 repository at \url{https://github.com/Populustremuloides/semantic_convergence} contains counterparts for the manuscript items over the explicit concrete foundation used in the repo, and publishes generated coverage, proof-shape, exactness-lock, and per-declaration axiom audits documenting the verification state. On the variational side, \path{SemanticConvergence.def_afe}, \path{SemanticConvergence.def_two_observer_functional}, and \path{SemanticConvergence.def_kernel_functional} realize the repaired Gibbs belief, class, and kernel objectives directly, while \path{SemanticConvergence.lem_variational}, \path{SemanticConvergence.lem_kl_necessity}, \path{SemanticConvergence.prop_two_observer_minimizer}, and \path{SemanticConvergence.prop_kernel_functional_minimizer} certify the corresponding exact Bayes/Gibbs variational identity, countable convex-duality KL-necessity theorem, and Gibbs minimizer results on the paper-facing countable stack. In Section~\ref{sec:main}, the H7 main theorem is represented by \path{SemanticConvergence.thm_constructed_infinite_bayes_gibbs_trueLaw_hellinger_spine_convergence_of_constrained_kernel_functional_minimizers_classPredictive_predictiveKernelObserver}; its semantic bridge uses \path{SemanticConvergence.countablePredictiveKernelObserver} and the predictive-kernel-observer calibration lemmas to avoid a separate semantic-coherence assumption. The concrete declarations \path{SemanticConvergence.thm_separating_support_convergence}, \path{SemanticConvergence.thm_separating_support_rate}, and \path{SemanticConvergence.cor_separating_support_finite_time} remain as a stronger zero-out support/rate route with finite-horizon bounds and separate exactness locks. At the boundary and surrogate end, \path{SemanticConvergence.thm_afe_near_miss}, \path{SemanticConvergence.thm_amortized_surrogate_selector_existence}, \path{SemanticConvergence.thm_amortized_surrogate}, and \path{SemanticConvergence.cor_amortized_surrogate_finite_time} expose the repaired action-level near-miss, countable \textup{(A1)--(A3)} wrapper, deployment-side support floor, and finite-time consequence. For any interactive learner, classical or modern, this H7 copy reduces the verified convergence question to a precise check: identify the instance $\mathcal I$, the minimized functional $\mathcal J$, and the target semantic set $\Scal(p^\star)$, then verify the consolidated assumptions before Theorem~\ref{thm:separating-support-convergence}.
