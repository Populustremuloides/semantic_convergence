...
\large H6 Lean-Verified Interface Draft}
\author{}
\date{}
...
\begin{center}
\fbox{\parbox{0.93\textwidth}{\textbf{H6 status.}
...
semantic-convergence claim is stated at the currently verified
Hellinger-spine interface: once the Bayes/Gibbs residual process satisfies the
named Hellinger martingale spine, posterior mass on the semantic class
converges to one. The remaining hardening task is the internal derivation of
that spine from the Bayes/Gibbs infinite trajectory law, semantic
Bhattacharyya separation, and realized policy support.}}
\end{center}

\begin{abstract}
We seek sufficient conditions under which an interactive learner's posterior concentrates on the \emph{interventional semantic class} $[p^\star]$ of an unknown computable environment --- the programs that induce the same action-conditioned law as $p^\star$ under every admissible intervention, and the strongest target any history-based learner can reach. The nested chain $\{p^\star\}\subseteq[p^\star]\subseteq R_\pi(p^\star)\subseteq R_{h_t}(p^\star)$ collects the indistinguishability classes of four \emph{observers} at increasing observational power, and each learning rule in the paper is an observer-promotion attempt. The paper studies the \emph{algorithmic free energy}: a Gibbs-form variational functional whose reference measure is the Kolmogorov-complexity-weighted universal interactive prior of \citet{Solomonoff1964a,Solomonoff1964b,Kolmogorov1965,Hutter2005}. Its belief-side minimization is exactly Bayesian updating with the KL divergence forced by convex duality, and its zero-cost uniform-preference action term recovers the information-gain criterion shared by Bayesian experimental design, compression-based curiosity, and active-inference curiosity. That generic information-gain route can still fail: a substituted finite-support near-miss freezes the posterior on $[p^\star]$ at prior mass for every finite horizon.

The verified positive theorem in this H6 copy targets the right observer geometry through an explicit Hellinger/Bhattacharyya martingale spine. If the Bayes/Gibbs residual odds process on the infinite realized trajectory has nonnegative residual odds, an $L^1$-bounded Hellinger envelope martingale, divergent cumulative observer-fiber Bhattacharyya separation, and the square-root Bayes identity, then the posterior on $[p^\star]$ converges almost surely to one. The Lean development also proves an H5 bridge showing that an all-time semantic affinity ceiling on policy-supported actions plus all-time realized policy support supplies the divergent-separation leg of that spine. What is \emph{not} claimed in this verified copy is that the paper's original separating-support hypotheses alone have already been proved to generate the Bayes/Gibbs Hellinger spine internally.

A public Lean 4 repository at \url{https://github.com/Populustremuloides/semantic_convergence} contains Lean counterparts for the manuscript items over the explicit concrete foundation used in the repo, together with generated coverage, proof-shape, exactness-lock, and per-declaration axiom audits. On the belief and variational side, \path{SemanticConvergence.def_afe}, \path{SemanticConvergence.def_two_observer_functional}, and \path{SemanticConvergence.def_kernel_functional} implement the repaired Gibbs objectives, while \path{SemanticConvergence.lem_variational}, \path{SemanticConvergence.lem_kl_necessity}, \path{SemanticConvergence.prop_two_observer_minimizer}, and \path{SemanticConvergence.prop_kernel_functional_minimizer} certify the corresponding exact variational identities and minimizer theorems. Around Section~\ref{sec:main}, the verified soft route is explicit: \path{SemanticConvergence.HellingerConvergenceSpine} packages the martingale spine, \path{SemanticConvergence.thm_separating_support_convergence_hellinger_spine} proves almost-sure posterior-share convergence from that spine, and \path{SemanticConvergence.hellingerConvergenceSpine_of_infiniteObserverFiberBhattacharyya_affinityCeiling_policySupport} is the H5 infinite-stream constructor reducing the divergence leg to semantic affinity ceiling plus all-time realized support. The concrete support/rate declarations \path{SemanticConvergence.thm_separating_support_convergence}, \path{SemanticConvergence.thm_separating_support_rate}, and \path{SemanticConvergence.cor_separating_support_finite_time} remain as a stronger zero-out special-case route with its own update/support-separation exactness locks.
\end{abstract}
...
This H6 copy answers the boxed problem at the verified Hellinger-spine interface. Theorem~\ref{thm:separating-support-convergence} says that the posterior on $[p^\star]$ converges almost surely to one whenever the Bayes/Gibbs residual odds and observer-fiber Bhattacharyya processes satisfy the named spine. The exact minimizing class-action kernel of the kernel lift is still the strongest variational candidate for generating that spine, and the class-against-complement rule is still the canonical selector candidate, but this verified copy does not claim that either candidate has yet been proved to construct the spine from the original separating-support assumptions alone. The older support/rate route is retained below as a concrete zero-out special case and as the roadmap for future hardening; its sharpness results still explain the two failure modes, wrong-observer geometry and insufficient separating support.

Section~\ref{sec:setup} fixes the interactive setting. Section~\ref{sec:hierarchy} constructs the four-set hierarchy, proves the observable-quotient theorem, and witnesses the three strict inclusions. Section~\ref{sec:functional} introduces the two-observer functional $\mathcal J_t^{\omega_B,\omega_A}$ together with its exact action-realizing kernel lift $\mathcal J_{t,\mathrm{ker}}^{\omega_B,\omega_A}$, and proves the meeting-point theorem. Section~\ref{sec:belief} identifies the belief-side minimizer at the canonical belief observer. Section~\ref{sec:semantic-action} identifies the exact kernel action law and the canonical class-against-complement selector realization at the canonical action observer, and proves that the separation geometry transfers through decodable observation channels. Section~\ref{sec:main} states the verified Hellinger-spine convergence theorem, records the H5 infinite-stream bridge that supplies its divergence leg from semantic affinity ceiling plus all-time realized support, and keeps the stronger support/rate stack as a conditional special-case route. Section~\ref{sec:self-ref} replaces the uncomputable behavioral observer by a finite-time proxy and records how its guarantees depend on the same convergence interface. Section~\ref{sec:near-miss} reads the algorithmic free energy principle as the boundary example of the functional outside the action-admissible range, and proves the near-miss. Section~\ref{sec:implications} records a differentiable surrogate loss (Section~\ref{sec:amortized-surrogate}) whose deployment-side guarantee is conditional on the same support/spine interface. Sections~\ref{sec:discussion} and~\ref{sec:conclusion} draw consequences.
...
Section~\ref{sec:main} establishes the verified \emph{convergence payoff}: the Hellinger-spine theorem proves posterior concentration from the exact martingale/Bhattacharyya interface, and the H5 infinite-stream bridge identifies a sufficient semantic-affinity/all-time-support route to the divergent-separation leg. The older support theorem, rate theorem, selector-free kernel theorem, canonical selector theorem, compact-action extension, and matched converses are retained as the conditional support-route program that the Lean development is still hardening below that spine.
...
Section~\ref{sec:functional} fixed the canonical action observer at $\omega_A=\omega_{\mathrm{behav}}$ (Corollary~\ref{cor:canonical-pair}) and Section~\ref{sec:belief} fixed the canonical belief observer at $\omega_B=\omega_{\mathrm{syn}}$; this section states the verified convergence interface for what happens once the learner acts there. Theorem~\ref{thm:separating-support-convergence} answers the boxed problem at the Hellinger-spine level: if the Bayes/Gibbs residual odds process has the Hellinger martingale spine, then the posterior on $[p^\star]$ converges almost surely to one. The H5 bridge below records the verified semantic route into one leg of that spine: a uniform observer-fiber Bhattacharyya affinity ceiling on every policy-supported action, together with all-time realized policy support, gives divergent cumulative separation on the infinite realized trajectory.

The rest of the section retains the original support/rate program as a conditional route and as a map of the remaining exactness work. In particular, statements that apply the older separating-support theorem should be read in this H6 copy as conditional on also supplying the Hellinger spine, or as belonging to the stronger concrete zero-out route tracked separately in the Lean manifest. This is the deliberate difference between this verified copy and the original inspirational manuscript.
...
\begin{theorem}[Hellinger-spine sufficient condition for semantic recovery]
\label{thm:separating-support-convergence}
Assume the universal prior and Bayes/Gibbs belief \textup{(C2)}. Fix a policy $\pi$ and a target program $p^\star\in\Pcal$, write
\[
c^\star:=[p^\star],
\qquad
...
\qquad
\text{almost surely on }E_t.
\]
Suppose in addition that the Bayes/Gibbs residual odds process for $c^\star$ admits a Hellinger convergence spine in the sense of Definition~\ref{def:hellinger-spine}. Then
\[
...
\begin{proof}
The support lower bound is the original support-route input; in this H6 verified version the additional load-bearing hypothesis is the Hellinger spine. Lemma~\ref{lem:contraction}, applied with $c^\star=[p^\star]$, gives the conclusion.
\end{proof}
...
\begin{remark}[H6 verification boundary for the support route]
\label{rem:h6-support-boundary}
The original manuscript's support theorem used a weaker stochastic hypothesis:
divergent cumulative policy mass on a set of separating actions. The H5 Lean
bridge above proves a stronger but cleaner all-time route: every
policy-supported action has a uniform affinity ceiling, so every realized step
has a positive Bhattacharyya floor. Closing the gap from the original support
hypothesis to the Hellinger spine requires an additional infinite-process
Borel--Cantelli and martingale-construction argument. Until that argument is
formalized, downstream support/rate specializations in this H6 copy should be
read as conditional on the Hellinger spine or as belonging to the separate
zero-out special-case route tracked in the Lean manifest.
\end{remark}
...
The boxed problem has two logically distinct parts, and this H6 copy resolves them at different levels. The belief rule is fully characterized: the Bayes/Gibbs posterior under the universal prior is the unique minimizer of the algorithmic free energy (Lemma~\ref{lem:variational}), and within a natural convex divergence class the KL-based functional is the only one with that property (Lemma~\ref{lem:kl-necessity}). The action/convergence rule is verified at the Hellinger-spine interface (Theorem~\ref{thm:separating-support-convergence}) and has an H5 infinite-stream bridge from semantic affinity ceiling plus all-time support (Theorem~\ref{thm:infinite-affinity-hellinger-bridge}). The stronger original support route remains the roadmap: Theorem~\ref{thm:afe-near-miss} fails by using the wrong observer geometry, while Theorems~\ref{cor:support-necessary} and~\ref{thm:summable-support-insufficient} fail in a different register --- the action rule targets the right observer, but the realized trajectory withholds enough mass from genuinely separating actions. The action objective supporting semantic recovery must therefore be class-targeted and sufficiently supported along the realized trajectory, and the remaining formal work is to derive the Bayes/Gibbs Hellinger spine from that support geometry.
...
The four nested sets $\{p^\star\}\subseteq[p^\star]\subseteq R_\pi(p^\star)\subseteq R_{h_t}(p^\star)$ are the indistinguishability classes of four observers at increasing observational power, with all three inclusions strict in general. The belief problem is solved exactly: the universal prior and Bayes/Gibbs posterior minimize the algorithmic free energy, and at zero action cost with uniform preferences the resulting expected free energy is the information-gain criterion shared by Bayesian experimental design, compression-based curiosity, and active-inference curiosity. Even this architecture can fail: the near-miss theorem (Theorem~\ref{thm:afe-near-miss}, Theorem~\ref{thm:observer-promotion-failure}) exhibits a substituted-prior family in which the realized policy induces an observer strictly coarser than the behavioral one, so the posterior on $[p^\star]$ need not concentrate. Theorem~\ref{thm:separating-support-convergence} gives the verified positive answer at the Hellinger-spine interface: once the Bayes/Gibbs residual process has the $L^1$-bounded Hellinger martingale envelope and divergent cumulative Bhattacharyya separation, the posterior on $[p^\star]$ converges almost surely to one. The H5 bridge, Theorem~\ref{thm:infinite-affinity-hellinger-bridge}, shows one verified semantic route into the divergence leg. The class-against-complement selector, exact Gibbs kernel lift, compact-action extension, and differentiable surrogate remain the canonical action-side candidates for constructing the spine, with the older support/rate arguments retained as conditional and zero-out special-case routes. The condition is sharp in the original support geometry: zero separating support makes uniform recovery impossible (Corollary~\ref{cor:support-necessary}) and summable separating support still admits failure with positive probability (Theorem~\ref{thm:summable-support-insufficient}), so the paper isolates two distinct ways semantic recovery can fail --- wrong-observer geometry and insufficient support on separating interventions.
...
The public Lean 4 repository at \url{https://github.com/Populustremuloides/semantic_convergence} contains counterparts for the manuscript items over the explicit concrete foundation used in the repo, and publishes generated coverage, proof-shape, exactness-lock, and per-declaration axiom audits documenting the verification state. On the variational side, \path{SemanticConvergence.def_afe}, \path{SemanticConvergence.def_two_observer_functional}, and \path{SemanticConvergence.def_kernel_functional} realize the repaired Gibbs belief, class, and kernel objectives directly, while \path{SemanticConvergence.lem_variational}, \path{SemanticConvergence.lem_kl_necessity}, \path{SemanticConvergence.prop_two_observer_minimizer}, and \path{SemanticConvergence.prop_kernel_functional_minimizer} certify the corresponding exact Bayes/Gibbs variational identity, countable convex-duality KL-necessity theorem, and Gibbs minimizer results on the paper-facing countable stack. In Section~\ref{sec:main}, the soft theorem is represented explicitly: \path{SemanticConvergence.HellingerConvergenceSpine} packages the residual nonnegativity, L1-bounded martingale envelope, divergent cumulative Bhattacharyya separation, and square-root Bayes identity; \path{SemanticConvergence.thm_separating_support_convergence_hellinger_spine} proves posterior-share convergence from that package; and \path{SemanticConvergence.hellingerConvergenceSpine_of_infiniteObserverFiberBhattacharyya_affinityCeiling_policySupport} supplies the H5 infinite-stream constructor from semantic affinity ceiling plus all-time realized support. The concrete declarations \path{SemanticConvergence.thm_separating_support_convergence}, \path{SemanticConvergence.thm_separating_support_rate}, and \path{SemanticConvergence.cor_separating_support_finite_time} remain as a stronger zero-out support/rate route with finite-horizon bounds and separate exactness locks. The manifest's stricter \texttt{fullyFirstPrinciples} flag remains false because the final internal construction of the Bayes/Gibbs-induced Hellinger spine from semantic separation and realized policy support is still open. At the boundary and surrogate end, \path{SemanticConvergence.thm_afe_near_miss}, \path{SemanticConvergence.thm_amortized_surrogate_selector_existence}, \path{SemanticConvergence.thm_amortized_surrogate}, and \path{SemanticConvergence.cor_amortized_surrogate_finite_time} expose the repaired action-level near-miss, countable \textup{(A1)--(A3)} wrapper, deployment-side support floor, and finite-time consequence. For any interactive learner, classical or modern, this H6 copy reduces the verified convergence question to a precise check: construct the Hellinger spine for the Bayes/Gibbs residual process.
